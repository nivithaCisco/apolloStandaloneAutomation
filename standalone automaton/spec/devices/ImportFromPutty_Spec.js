...
describe('Import From Putty', function () {
    ...
    it('test ', function ()
    {
     ...
     it('Verify Import From Putty option is present', function ()
     {

     expect(upload.upload_arrow().isPresent()).toBe(true);
     upload.upload_arrow().click();
     expect(upload.import_from_putty().isPresent()).toBe(true);
         browser.sleep(5000);
     });

    it('Verify all the elements of Import From Putty modal is present ', function ()
    {   upload.import_from_putty().click();
        expect(upload.autoImport().isPresent()).toBe(true);
        expect(upload.manuaImport().isPresent()).toBe(true);
        upload.manuaImport().click();
    ...
    using(AppDataProvider.ImportFromPutty, function (data, description) {

        it('Import From Putty: ', function ()

        {
            ...
            {
                browser.sleep(2000);
                upload.uploadDevice(data.ToUpload, data.protocol,'ImportFromCSV' );
            }
          browser.sleep(2000);
            upload.uploadDevice(data.ToUpload, data.protocol,'ImportFromCSV');
            browser.sleep(2000);
        ...
        it('Import From Putty'+ description, function () {
            var exp;
            try {
